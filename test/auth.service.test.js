  ...
  describe('updateImage', () => {
    ...
    it('should delete old image and upload new one when file provided', async () => {
      const mockUser = {
        _id: 'user123',
        profileImageUrl:
          'http://res.cloudinary.com/demo/image/upload/v1234/oldpic.jpg',
      };
      userRepository.findById.mockResolvedValue(mockUser);
  ...
  describe('deleteUser', () => {
    it('should delete user and cascade delete expenses, income, and cloudinary assets', async () => {
      const mockUser = {
        _id: 'user123',
        profileImageUrl:
          'http://res.cloudinary.com/demo/image/upload/v1234/profile.jpg',
      };
      userRepository.findById.mockResolvedValue(mockUser);
      expenseService.deleteAllExpensesByUser.mockResolvedValue({
        deletedCount: 2,
      });
      incomeService.deleteAllIncomeByUser.mockResolvedValue({
        deletedCount: 1,
      });
      cloudinary.api.delete_resources.mockResolvedValue({});
      ...
      expect(expenseService.deleteAllExpensesByUser).toHaveBeenCalledWith(
        'user123'
      );
      expect(incomeService.deleteAllIncomeByUser).toHaveBeenCalledWith(
        'user123'
      );
      expect(cloudinary.api.delete_resources).toHaveBeenCalledWith(
