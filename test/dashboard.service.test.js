...
describe('Dashboard Service Unit Tests', () => {
  ...
  describe('getDashboardData', () => {
    it('should calculate and aggregate user dashboard metrics', async () => {
      ...
      expect(incomeRepository.findSinceDate).toHaveBeenCalledWith(
        'user123',
        expect.any(Date)
      );
      expect(expenseRepository.findSinceDate).toHaveBeenCalledWith(
        'user123',
        expect.any(Date)
      );
      ...
      expect(result.last30DaysExpenses.transactions).toEqual(
        expenseTransactions
      );
