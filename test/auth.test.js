...
describe("DELETE /api/auth/deleteUser", () => {
  ...
  it("should handle cloudinary deletion error gracefully", async () => {
    // Mock User.findById to return user with profile image and select method
    const mockUser = {
      _id: user._id,
      profileImageUrl:
        "https://res.cloudinary.com/test/image/upload/v1234567890/expense-tracker/test123.jpg",
    };
    ...
    // Mock Promise.all to succeed (expenses and income deletion)
    jest.spyOn(Promise, "all").mockResolvedValueOnce([]);

    // Mock cloudinary deletion to fail
    const mockCloudinaryError = new Error("Cloudinary deletion failed");
    jest
      .spyOn(require("../config/cloudinary").api, "delete_resources")
      .mockRejectedValueOnce(mockCloudinaryError);

    // Mock User.findByIdAndDelete to succeed
